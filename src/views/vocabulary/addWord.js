...
const AddWord = () => {
  ...
  const getVocabulary = async () => {
    try {
      ...
      const response = await fetch(`${serverUrl}/api/vocabulary/${id}/meta`, {
      ...
      wordCategories.map((wc) => {
        setWordCategoryOptions((wco) =>
          wco.concat({ l: t(`word_category_${wc.name}`), v: wc.id })
        );
      });
    } catch (error) {
  ...
  const addWord = async () => {
    try {
      ...
      const response = await fetch(
        `${serverUrl}/api/vocabulary/${id}/addWord`,
        {
          method: "POST",
          headers: {
    ...
    <Box align="center">
      ...
      >
        <FormField
          ...
          label={t(`language_${plainLanguageCode}`)}
        >
        ...
        <FormField
          name="translation"
          htmlfor="text-input-id"
          label={t(`language_${translatedLanguageCode}`)}
        >
